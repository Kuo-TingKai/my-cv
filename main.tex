...
\begin{tikzpicture}[scale=0.5]
    ...
    \node[anchor=west] at (6.2, -3.9) {\small Stream/FFmpeg};
...
{\footnotesize \textit{Role:} I am responsible for (1) machine control program development and integration with frontend/backend game events and error handling; (2) developing the core event synchronization logic in the streaming player; (3) developing the monitoring system for machine status.\par}
\vspace{0.4em}
\begin{itemize}[noitemsep]
    %--- Core product: multimedia \& streaming ---
    ...
    \item Websockets/\textbf{MQTT} for game state machine message passing to Database (\textbf{PostgreSQL, MongoDB}); MQTT also used to (1) integrate on-premise \textbf{computer vision} recognition and (2) drive \textbf{PLC} electromechanical actuation commands and status monitoring
    \item Serial port communication (USB, RS232) for game machine \textbf{micro-controller (MCU)}
    \item \textbf{Probability engineering}: Monte Carlo for event stochastic process outcome simulations; dice-shaker oscillation-mode randomness and feature design; Baccarat shoe shuffling pattern randomness monitoring
    ...
    %--- Tech stack ---
    \item Technology stacks: Python, C++, Typescript, JavaScript, Go for multimedia player, hardware device controller, monitoring system, and related applications
    %--- Process, tooling \& collaboration ---
    \item Spec-driven coding with \textbf{Cursor} and \textbf{.cursorrules} as constraints to deliver projects following software design patterns and clean code standards
    \item \textbf{GitHub Actions} workflow for automated deployment to target machines on each release and test run
    \item Familiar with \textbf{GitHub} code review and PR workflow to ensure code quality through open discussion and alignment between developers and leads on implementation
    \item \textbf{Jira} ticket management for feature development and bug-fixing progress to ensure transparency and traceability for the department, stakeholders, and PO
    \item Documenting with \textbf{Confluence}: skilled at writing technical documentation; use plugins such as content, changelog, \textbf{mermaid} to produce professional, highly readable documents
    \item \textbf{Co-working with Europe team} for game projects without obstacles
    \item Participated in \textbf{ICE Barcelona 2026} (Europe's largest iGaming exhibition)
\end{itemize}
